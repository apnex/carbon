	...
	switch(cmd) {
		case 'complete':
			if(args.length > 0) {
				...
				let string = chain(args.slice(1), 1);
				console.log('--[ ' + blue(string) + ' ]--');
				run(args.slice(1), cmdSpec, 1);
			} else {
		...
		case 'browse':
			if(args.length > 0) {
				let cmdSpec = require(args[0]);
				let string = chain(args.slice(1), 0);
				console.log('--[ ' + blue(string) + ' ]--');
				run(args.slice(1), cmdSpec, 0);
			} else {
				console.log('[' + orange('ERROR') + ']: command usage: ' + green('cmdrun browse') + ' ' + blue('<cmdfile.json> [ <cmds> ... ]'));
			}
		break;
		case 'generate':
			if(args.length > 1) {
...
function run(cmds, body, slurp = 1) {
	if(index < cmds.length) {
		...
		/*
		if(typeof(body) == "number") { // is an action
			let action = cmds.pop();

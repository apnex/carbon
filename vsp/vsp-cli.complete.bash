...
_vsp-cli_complete() {
	...
	if [[ ${PRV} != "list" ]]; then
		...
		if [[ ${#COMP_WORDS[@]} -ge 2 ]]; then
			ARRAY=($(vsp-cli "[]" "${COMP_WORDS[@]:1:${#COMP_WORDS[@]}-2}" 2>/dev/null | tr -d '\r')) # handle CRLF in tty
		else
			ARRAY=($(vsp-cli "[]" 2>/dev/null | tr -d '\r')) # handle CRLF in tty
		fi
...
complete -F _vsp-cli_complete vsp-cli
